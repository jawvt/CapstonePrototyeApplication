...
#install.packages(c("shiny", "bslib", "leaflet", "htmltools", "DT", "shinyjs", "base64enc", "googlesheets4", "googledrive"))

#################################################################################
# GOOGLE SHEETS SETUP (OPTIONAL - FOR PERSISTENT STORAGE):
################################################################################
# This demo version uses in-memory storage. Submissions are lost when app restarts.
# To enable persistent Google Sheets backend:
#
# 1. Uncomment the googlesheets4 and googledrive library() calls above
# 2. Create a new Google Sheet with two tabs:
#    - Tab 1: "Submissions" 
#    - Tab 2: "Approved"
#
# 3. In both tabs, create these column headers in row 1:
#    submission_id | name | email | painting_id | photo_url | latitude | 
#    longitude | observations | submission_date | approval_status
#
# 4. Get your Google Sheet ID from the URL and update the GOOGLE_SHEET_ID variable
#
# 5. Set up authentication (see configuration section above)
#
# 6. Replace the in-memory storage code in server function with Google Sheets calls
################################################################################
...
################################################################################
# CONFIGURATION - SIMPLIFIED LOCAL STORAGE
################################################################################

# This demo version stores submissions in memory (they will be lost when app restarts)
# For production use with persistent storage, uncomment Google Sheets section below
      ...
      div(class = "content-section",
          ...
          div(class = "form-section",
              div(class = "info-box",
                  icon("camera"),
                  strong(" Instructions: "),
                  "Visit one of Bierstadt's painting locations, take a photo from a similar vantage point, and fill out the form below. Note: For this demo version, submissions are stored locally in your browser only."
              ),
    ...
    tabPanel(
      ...
      div(class = "content-section",
          ...
          conditionalPanel(
            ...
            div(class = "info-box",
                icon("info-circle"),
                " Review submissions below. In the full version with Google Sheets backend, you would approve submissions by changing their status."
            ),
...
server <- function(input, output, session) {
  ...
  # Reactive values for in-memory storage
  rv <- reactiveValues(
    admin_auth = FALSE,
  ...
  observeEvent(input$submit_button, {
    ...
    tryCatch({
      ...
      # Add to submissions (in-memory storage)
    ...
    if (nrow(approved) == 0) {
      return(
        div(class = "info-box",
            ...
            p("Be the first to contribute! Visit a painting location and submit your photo. Note: In this demo, submissions are stored in memory only and will be lost when the app restarts. For production, enable the Google Sheets backend.")
        )
      )
    }
...
#    - Change the admin password (line with rv$admin_auth check)
#    - Optionally set up Google Sheets backend for persistent storage
#
# 4. Deploy:
#    rsconnect::deployApp()
#
# NOTE: This demo version uses in-memory storage, so submissions will be lost
# when the app restarts. For production use with persistent storage, set up
# the Google Sheets backend as described in the configuration section.
#
################################################################################